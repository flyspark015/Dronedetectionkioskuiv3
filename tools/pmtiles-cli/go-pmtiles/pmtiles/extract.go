...
func loadExtractState(path string) (*extractState, error) {
...
func saveExtractState(path string, state *extractState) error {
...
func Extract(logger *log.Logger, bucketURL string, key string, minzoom int8, maxzoom int8, regionFile string, bbox string, output string, downloadThreads int, overfetch float32, dryRun bool) error {
	...
	ranges := make([]overfetchRange, 0, overfetchRanges.Len())
	for e := overfetchRanges.Front(); e != nil; e = e.Next() {
		ranges = append(ranges, e.Value.(overfetchRange))
	}
	numOverfetchRanges := len(ranges)
	fmt.Printf("fetching %d tiles, %d chunks, %d requests\n", len(reencoded), len(tileParts), numOverfetchRanges)
	...
	if !dryRun {
		...
		state, stateErr := loadExtractState(statePath)
		resume := false
		if stateErr == nil && state != nil && state.TotalBytes == totalBytes {
		...
		completedList := make([]int, 0, len(ranges))
		bytesDownloaded := uint64(0)

		if resume && state != nil {
			for _, idx := range state.Completed {
				if idx >= 0 && idx < len(ranges) && !completed[idx] {
					completed[idx] = true
					completedList = append(completedList, idx)
					bytesDownloaded += ranges[idx].Rng.Length
				}
			}
		} else {
			state = &extractState{Version: 1, TotalBytes: totalBytes}
			_ = os.Remove(statePath)
		}

		expectedSize := 127 + int64(len(newRootBytes)) + int64(header.MetadataLength) + int64(len(newLeavesBytes)) + int64(totalActualBytes)
		...
		var bytesDownloadedAtomic uint64
		atomic.StoreUint64(&bytesDownloadedAtomic, bytesDownloaded)

		var completedMu sync.Mutex
		writeState := func() {
			completedMu.Lock()
			snapshot := make([]int, len(completedList))
			copy(snapshot, completedList)
			completedMu.Unlock()
			state.Completed = snapshot
			state.TotalBytes = totalBytes
			state.BytesDownloaded = atomic.LoadUint64(&bytesDownloadedAtomic)
			state.UpdatedTs = time.Now().Unix()
			_ = saveExtractState(statePath, state)
		}

		logProgress := func() {
			bd := atomic.LoadUint64(&bytesDownloadedAtomic)
			percent := 0.0
			if totalBytes > 0 {
				percent = (float64(bd) / float64(totalBytes)) * 100.0
			}
			elapsed := time.Since(start)
			eta := "unknown"
			if elapsed.Seconds() > 0 && bd > 0 && totalBytes > bd {
				rate := float64(bd) / elapsed.Seconds()
				remaining := float64(totalBytes-bd) / rate
				eta = (time.Duration(remaining) * time.Second).Truncate(time.Second).String()
			}
			logf("PROGRESS bytes=%d total=%d percent=%.2f eta=%s", bd, totalBytes, percent, eta)
		}
		...
		downloadPart := func(or overfetchRange) error {
			tileReader, err := bucket.NewRangeReader(egCtx, key, int64(sourceTileDataOffset+or.Rng.SrcOffset), int64(or.Rng.Length))
			if err != nil {
				return err
			}
			defer tileReader.Close()
			offsetWriter := newOffsetWriter(outfile, int64(header.TileDataOffset)+int64(or.Rng.DstOffset))

			for _, cd := range or.CopyDiscards {
				if _, err := io.CopyN(offsetWriter, tileReader, int64(cd.Wanted)); err != nil {
					return err
				}
				if cd.Discard > 0 {
					if _, err := io.CopyN(io.Discard, tileReader, int64(cd.Discard)); err != nil {
						return err
					}
				}
			}
		...
		workCh := make(chan int, len(ranges))
		...
		for i := 0; i < downloadThreads; i++ {
			eg.Go(func() error {
				for idx := range workCh {
					...
					if err := downloadPart(ranges[idx]); err != nil {
						return err
					}
					completedMu.Lock()
					if !completed[idx] {
					...
					completedMu.Unlock()
					atomic.AddUint64(&bytesDownloadedAtomic, ranges[idx].Rng.Length)
				}
				return nil
			})
		}
